...
#include "GURUX_DLMS/Servers/GXDLMSServer.h"
#include "GURUX_DLMS/GXBitString.h"
...
int CGXDLMSAssociationLogicalName::GetAccessRights(CGXDLMSObject* pItem, CGXDLMSServer* server, CGXByteBuffer& data)
{
    ...
    for (int pos = 0; pos != cnt; ++pos)
    {
        e.SetIndex(pos + 1);
        index = pos + 1;
        access = this->GetAttributeAccess(server, e);
    ...
    for (int pos = 0; pos != cnt; ++pos)
    {
        e.SetIndex(pos + 1);
        index = pos + 1;
        access = this->GetMethodAccess(server, e);
...
DLMS_ACCESS_MODE CGXDLMSAssociationLogicalName::GetAttributeAccess(CGXDLMSServer *server, CGXDLMSValueEventArg &e) {
    if (server != NULL) return server->GetAttributeAccess(&e);
    else                return DLMS_ACCESS_MODE_READ_WRITE;
}

DLMS_METHOD_ACCESS_MODE CGXDLMSAssociationLogicalName::GetMethodAccess(CGXDLMSServer *server, CGXDLMSValueEventArg &e) {
    if (server != NULL) return server->GetMethodAccess(&e);
    else                return DLMS_METHOD_ACCESS_MODE_ACCESS;
}
