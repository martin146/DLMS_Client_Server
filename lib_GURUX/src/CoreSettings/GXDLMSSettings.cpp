...
CGXDLMSObjectCollectionE& CGXDLMSSettings::GetObjects()
{
    return m_Objects;
}
...
{
    //** added **/
    CGXDLMSAssociationLogicalName *ln = (CGXDLMSAssociationLogicalName*) this->m_Objects.FindByLN(DLMS_OBJECT_TYPE_ASSOCIATION_LOGICAL_NAME, this->m_Objects.getActAssociationLn());
    std::string securitySetupLn = ln->GetSecuritySetupReference();
    CGXDLMSSecuritySetup *securitySetup = (CGXDLMSSecuritySetup*) this->m_Objects.FindByLN(DLMS_OBJECT_TYPE_SECURITY_SETUP, securitySetupLn);
    securitySetup->SetClientSystemTitle(value);
